...
# Applying an assumed a fixed fee percentage of 0.2% for calculations
...
def calculate_yearly_fee(investment_amount, investment_date):
    ...
    # Return 0 for investment without 1 year of completion
    if year == 0:
        return Decimal(0)
    ...
    if investment_date < CUTOFF_DATE:
        if year == 1:
            days_in_year = Decimal(
                365 if investment_date.year % 4 != 0 else 366)
            fraction_of_year = Decimal(
                investment_date.timetuple().tm_yday) / days_in_year
            return fraction_of_year * fee_percentage * investment_amount
        else:
